...
FILE* UDF_OUTPUT_FILE(char *fpath, char *fname);
...
DEFINE_SOURCE(xmom_source, c, t, dS, eqn) {
	...
	source_x = source;
	/*	Message(cannot read property of tolowercase debug
	...
	char *fappendfix=NULL;
	strcpy(ufpath, UDF_OUT_FILE_PATH);
	strcpy(ufname, UDF_OUT_NAME);
	sprintf(fappendfix,"-%d",N_TIME);
	strcat(ufname,fappendfix);
	udfp = UDF_OUTPUT_FILE(ufpath, ufname);
	fprintf(udfp,
			"your porous air_vc adsf pd gmd gsd fvx flx source is %g %g %g %g %g %g %g %g %g \n",
			porous, air_vc, adsf, pd, gmd, gsd, fvx, flx, source);
	fclose(udfp);
...
DEFINE_SOURCE(ymom_source, c, t, dS, eqn) {
	...
	if (FLAG_OUT_PUT)
	{
		...
		char *fappendfix1=NULL;
		strcpy(ufpath1, UDF_OUT_FILE_PATH);
		strcpy(ufname1, UDF_OUT_NAME);
		sprintf(fappendfix1,"-%d",N_TIME);
		strcat(ufname1,fappendfix1);
		udfp1 = UDF_OUTPUT_FILE(ufpath1, ufname1);
		fprintf(udfp1,
				"your porous air_vc adsf pd gmd gsd fvy fly source is %g %g %g %g %g %g %g %g %g \n",
				porous, air_vc, adsf, pd, gmd, gsd, fvy, fly, source);
		fclose(udfp1);
...
real PRESSURE_DROP_SOURCE(real c_porousity, real vcsity,
	...
	tpc_drop = acoef * pow(bcoef, -1) * fabs(face_velocity)
			* fabs(face_dust_load);
...
FILE* UDF_OUTPUT_FILE(char *fpath, char *fname) {
	...
	char *fullname = (char*) malloc(
			(MAX_OUTPUT_UDF_FILEPATH + MAX_OUTPUT_UDF_FILENAME) * sizeof(char));
